...
import { parseAddressInfo } from '@/services/api/shipment'
 ...
 * @returns
 */
export async function exportExcel({ jsonData, theadColumns, filename, canDataEmpty = false }) {
    ...
    if (result) {
      ...
      if (res.code === 0) {
        ...
        if (res.data && res.data.length > 1) {
          ...
          return {
            msg: '导入的数据为空,请确认',
            header: (res.data && res.data[0]) || []
          }
        }
        return {
          jsonData,
          header: res.data[0],
          fileCenterParams: params
        }
      }
      let msg = ''
      switch (res.code) {
        case 3003:
          // `一次最多只允许导入${maxRowLimit}条，请修改Excel`
          msg = '一次最多只允许导入1000条，请修改Excel'
          break
        case 3004:
...
async function readExcelByJs(file, headerIndex = 0) {
  if (await native.isClientMode()) {
    if (await native.existsNativeFunc('readExcelAsync')) {
      return new Promise(async resolve => {
        window.webApi_getReadExcelResult = data => {
          resolve(data)
        }
        await native.readExcelAsync(file.name, headerIndex, () => {})
      })
    }
    const jsonData = JSON.parse(await native.readExcel(file.name, headerIndex))
    return jsonData
  }
  const excel = await import('@utils/excelUtil')
...
async function readFileToBase64ByFileReader(file, removeFileInfo) {
 ...
 */
export async function readFileToBase64(file, removeFileInfo) {
  ...
  const base64 = await readFileToBase64ByFileReader(file, removeFileInfo)
  return base64
}
 ...
 */
export async function printByCloudPrinter(
  waybills,
  printerNo,
  sendStubChecked,
  childMotherChecked,
  ...
  if (!sheetType || sheetType === '1') {
    res = await batchPrint_210(
      models,
      printerNo,
      store.getters.encryptionText,
      sendStubChecked,
      childMotherChecked
    )
...
export function convertPrintModelsForLocalPrint(waybills) {
  return waybills.map(val => ({
    ...
    jjTime: val.jjDateTime,
    receiptAmount: val.receiptAmount || val.ReceiptAmount,
    ...
    mjAmount: val.mjAmount || val.MJAmount,
    dsHKAmount: val.dsHKAmount || val.DSHKAmount,
    bfAmount: val.bfAmount || val.BFAmount,
...
export function convertQueryModels(waybills) {
  ...
  const result = waybills.map(val => ({
    ...
    sizeList:
      ...
      val.preWaybillGoodsSizeList.map(m => ({
        width: m.width,
        len: m['length'],
        height: m.height,
        number: m.count
    ...
    SizeText:
      val.preWaybillGoodsSizeList &&
      val.preWaybillGoodsSizeList
        .map(m => m.length + '*' + m.width + '*' + m.height + '*' + m.count)
        .toString(),
...
export function formatTelephoneOfModel(model) {
  ...
  const sendeLandline = `${model.jjTelephoneAreaNo || ''}-${model[jjTelephoneName] || ''}-${
    model.jjTelephoneExtension || ''
  }`
  model[jjTelephoneName] = trim(sendeLandline, '-') || ''
  const receiverLandline = `${model.sjTelephoneAreaNo || ''}-${model.sjTelephone || ''}-${
    model.sjTelephoneExtension || ''
  }`
...
export function convertEncryptListToString(fieldsList) {
  ...
  ;[10, 20, 30].forEach(type => {
    ...
    fields.forEach(field => {
      let encryptType = fieldsList.find(
        f => f.titleType == type && f.fieldName == field
      ).encryptType
...
// //下载批量导入模板
// export async function downloadImportExcelTemplate(isSignBySelf) {
...
//   const res = await native.downLoadFile(url, 'excel', fileName)
...
//下载批量导入模板
export async function downloadImportExcelTemplate() {
  const isSignBySelf = store.getters.isSignBySelf
  let fileName = '跨越批量发货模板'+ dayjs().format('YYYY年MM月DD日')
  if (isSignBySelf) {
    fileName += '(本人签收)'
  }
  fileName += '.xls'
  const urlRes = await deliveryApi.getBatchImportTemplateUrl(isSignBySelf)
  if(urlRes.code !== 0){
    Message.error(urlRes.msg || '模板获取失败')
    return
  }
  store.commit('delivery/setExistsNewImportTemplate',false)
...
function recognizeName(wordsList, inputText) {
  ...
  let nameList = wordsList.filter(
    w => w.endsWith('先生') || w.endsWith('小姐') || familyName.includes(w.substr(0, 1))
  )
  ...
  return {
    name: nameList[0],
    inputText: inputText.replace(new RegExp(nameList[0], 'g'), ' ')
  }
}

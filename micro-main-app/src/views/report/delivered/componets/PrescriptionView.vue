<template>
  <div class="prescription-overview-container container card-wrapper">
    <div class="caption-info">
      <card-title title="时效内妥投分析"/> 
      <slot name="tip"></slot>
      <div class="detail" @click="$router.push({path:'report/PrescriptionIndex',query: { dateRange: JSON.stringify(dateRange) }})">详情<i class="el-icon-arrow-right"></i></div>
    </div>
    <div class="content">
      <div class="top">
      ...
      <div class="detail-info">
        <el-tabs v-model="activeName" @tab-click="loadTabData">
          <el-tab-pane label="产品服务TOP10" name="first">
            <el-table :data="tableData" style="width: 100%" :highlight-current-row="false" ref="serviceModelTable">
              <div slot="empty">
...
import { getPrescriptionSummary, getPrescriptionByServiceModeList, getPrescriptionByAddressList } from '@/services/api/report'
...
export default {
  ...
  methods: {
    async loadData () {
      ...
      const res = await getPrescriptionSummary(params)
      if (res.code === 0) {
    ...
    async getInvestmentProviceOrCityList (type, search) {
      ...
      let arr = []
      const res = await getPrescriptionByAddressList(params)
      if (res.code === 0) {
    ...
    async loadTabData(){
      if(this.activeName === 'first'){
        ...
        let arr = []
        const res2 = await getPrescriptionByServiceModeList(params)
        if (res2.code === 0) {

      ...
      <el-row>
        <el-col :span="span">
          <el-form-item :error="errorMsgOfdistrict">
            <tabs-cascader :props="tabsCascaderProps" :labelList="formData.districtList" @valueChange="changeDistrict" :fetch-suggestions="getMultiLevelThinkList" @addressLoaded="addressLoaded" ref="cascaderRef" :placeholder="tabCascaderPlaceholder" />
          </el-form-item>
        </el-col>
        <el-col :span="span" v-if="showInput">
          <el-form-item prop="detailAddress">
            <encrypt-field-input v-if="encryptDetailAddress.visible" placeholder="请输入详细地址" :encryptField="encryptDetailAddress" />
            <el-autocomplete v-else v-model="formData.detailAddress" :title="formData.detailAddress" size="medium" popper-class="address-autocomplete" clearable :trigger-on-focus="false" :debounce="500" :fetch-suggestions="addressSuggest" @select="selectSuggestAddress" :maxlength="80" placeholder="请输入详细地址" @change="handleDetailAddressChange">
...
<script>
  ...
  export default {
    ...
    data () {
      ...
      return {
        errorMsgOfdistrict: '',
        encryptDetailAddress: {
          value: '',
          visible: false,
          content: '',
          replace: val => val.replace(/./g, "*")
        },
        formData: {
          districtList: [],
          detailAddress: ""
    ...
    watch: {
      ...
      'formData.detailAddress': {
        handler: function () {
          this.$nextTick(() => {
            if (this.encryptField && this.encryptField.visible) {
              this.encryptDetailAddress.value = this.formData.detailAddress
            }
          })
        },
        immediate: true
      },
      'encryptDetailAddress.visible' (val) {
        if (!val && this.encryptField) {
          this.encryptField.visible = val
        }
      },
      'encryptField': {
        handler (val) {
          ...
          this.encryptDetailAddress.visible = val.visible
          this.encryptDetailAddress.content = val.content
        },
        immediate: true,
        deep: true
      }
    ...
    methods: {
      ...
      async handleDetailAddressChange () {
        try {
          await this.$refs.form.validate()
          if(this.formData.detailAddress && this.formData.districtList?.length>0){
            const districtText = this.formData.districtList?.join('') || ''
            const address = districtText + this.formData.detailAddress.replace(districtText)
             await this.splitTheAddress(address)
          }else
      ...
      async selectSuggestAddress (addressInfo) {
        ...
        if (districtList.filter(f => f).length < 3) {
          ...
          // if (this.formData.districtList.length > 0 && this.formData.districtList != districtList) {
          //   await this.$confirm(`您选择的地址可能属于${districtList.join('')},是否切换地址`, '温馨提示')
          // }
      ...
      getAddressInfo (allowEmpty = false) {
         let hasEmpty = false
         const selectedOptions = this.$refs.cascaderRef.selectedOptions
         if(!allowEmpty){
           selectedOptions.map((m, index)=>({label: m.label, id: m.value}))
         }else{
          return [1,2,3,4].map(i=>{
             const item = selectedOptions.find(f=>f.value?.startsWith(`XZ${i}`))
             if(item){

...
const KyDragDialog = Vue.component('ky-drag-dialog', {
  functional: true,
  render(h, self) {
    ...
    if (!self.props.hasOwnProperty('appendToBody') || self.props.appendToBody === false) {
      ...
      const wrapperClassName = 'el-dialog__wrapper kye-dialog__modal-in-tab'
      self.data.staticClass = self.data.staticClass
        ? `${wrapperClassName} ${self.data.staticClass}`
        : wrapperClassName
    }

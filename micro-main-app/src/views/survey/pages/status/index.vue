  ...
  <div class="status">
    ...
    <div class="content">
      <div class="success" v-if="+status === 1">
        ...
        <div v-if="integeNum <= 0" class="tip__text">感谢您的参与</div>
        <div class="des" v-if="integeNum > 0">
          <div>
            1、您已获得<span class="light">{{ integeNum }}积分</span
        ...
        <div class="btn-result" @click="goResult">查看提交结果</div>
        <div class="erweima" v-if="integeNum > 0">
          <div class="img"></div>
          <div class="wx-text">微信扫一扫关注公众号</div>
        </div>
      </div>
      <div class="not-started" v-if="+status === 2">
...
export default {
  ...
  computed: {
    notStartTime() {
      return dayjs(parseInt(this.startTime)).format("YYYY-MM-DD HH:mm") || 0;
    },
  },
  created() {
    this.setStatus();
  ...
  methods: {
    setStatus() {
      const { id, status, startTime, integeNum } = this.$route.query;
      this.id = id;
      this.status = status;
      this.startTime = startTime;
      this.integeNum = integeNum;
    },
    goResult() {
      this.$router.push({

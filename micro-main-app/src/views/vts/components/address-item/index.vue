      ...
      <el-row>
        <el-col :span="span">
          <el-form-item :error="errorMsgOfdistrict">
            <tabs-cascader :props="tabsCascaderProps"
                           :labelList="formData.districtList"
                           @valueChange="changeDistrict"
                           :fetch-suggestions="getMultiLevelThinkList"
        ...
        <el-col :span="span"
          ...
          <el-form-item prop="detailAddress">
            <!-- <encrypt-field-input v-if="encryptDetailAddress.visible"
                                 placeholder="请输入详细地址"
                                 :encryptField="encryptDetailAddress" /> -->
            <el-autocomplete v-model="formData.detailAddress"
...
<script>
  ...
  import { splitAddress, getMultiLevelThinkList, getMultiLevelSubList, addressSuggestApi } from "@/services/api/vts"
  ...
  export default {
    ...
    methods: {
      ...
      async handleDetailAddressChange () {
        ...
        try {
          await this.$refs.form.validate()
          if (this.formData.detailAddress && this.formData.districtList && this.formData.districtList.length > 0) {
            const districtText = this.formData.districtList && this.formData.districtList.join('') || ''
      ...
      async selectSuggestAddress (addressInfo) {
        ...
        if (districtList.filter(f => f).length < 3) {
          ...
          if (this.formData.districtList.length > 0 && this.formData.districtList != districtList) {
            await this.$confirm(`您选择的地址可能属于${districtList.join('')},是否切换地址`, '温馨提示').then(() => {
      ...
      getAddressInfo (allowEmpty = false) {
        ...
        if (!allowEmpty) {
          ...
          return [1, 2, 3, 4].map(i => {
            const item = selectedOptions.find(f => f.value && f.value.startsWith(`XZ${i}`))
            if (item) {

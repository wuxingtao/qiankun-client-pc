            ...
            <div :ref="'routesContent'+index"
              ...
              <el-timeline>
                <el-timeline-item v-for="(route, itemIndex) in value"
                  ...
                  <span @click="changePhone(route)"
                        v-html="route.stepDesc">asd</span>
                </el-timeline-item>
              </el-timeline>
            </div>
          </div>
...
<script>
import { formateDate } from '../assets/common'
import { getRoutesApi } from "@/services/api/bill"
...
export default {
    data () {
        return {
            ...
            statusName: '',
            statusImgUrl: '',
            planeImgUrl: require("@/assets/image/bill/plane.png"),
            planeActiveImgUrl: require("@/assets/image/bill/plane-active.png"),
            phones: []
        }
    },
    created () {
    ...
    methods: {
        ...
        initRoutesInfo () {
            switch (this.routeInfo.ydStatus + '') {
                case '1':
                    ...
                    this.statusImgUrl = require("@/assets/image/bill/route-status-unpicked.png")
                    break
                case '2':
                    this.statusName = "待取货"
                    this.statusImgUrl = require("@/assets/image/bill/route-status-picking.png")
                    break
                case '3':
                    this.statusName = "待签收"
                    this.statusImgUrl = require("@/assets/image/bill/route-status-unsign.png")
                    break
                case '4':
                    this.statusName = "已签收"
                    this.statusImgUrl = require("@/assets/image/bill/route-status-signed.png")
                    break
                default:
                    this.statusName = '无'
            }
        ...
        formateTimeStamp (val) {
            return formateDate(new Date(val), 'HH:mm:ss')
        },
        ...
        getGroupedRoutes () {
            ...
            const groupedList = this.groupBy(this.routeInfo.route, item => formateDate(new Date(item.dateTime), "yyyy-MM-dd EEE"))
            return groupedList
        },
        groupBy (array, fn) {
            const groups = {}

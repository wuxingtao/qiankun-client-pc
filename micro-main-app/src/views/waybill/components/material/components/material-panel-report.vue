<template>
  <div class='material-panel-report' v-loading='loading'>
    <template v-if='images.length <= 0 && !loading'>
      <material-nodata />
    </template>
    <template v-else>
      <el-image
        class='material-panel-report-image'
        fit='scale-down'
        :src='images[0]'
  ...
  methods: {
    async createWaybillMaterial() {
      ...
      const params = {
        ...
        /** 只下载已签收状态 */
        waybillStatus: ['30', '40'],
        picType: downloadPicType.WAYBILL_UPLOAD_IMAGE_REPORT
      }
      API.createWaybillMaterial(params).then(res => {
        if (res.code === 0) {
          if (res.data > 0) {
            ...
            this.$message.warning('运单联下载失败，请稍后再试')
          }
        }
      }).catch(() => {
        this.downLoading = false
    ...
    async getWaybillMaterialUrl() {
      ...
      if (this.requestCount >= this.step) {
        clearTimeout(this.timer)
        this.downLoading = false
        return this.$message.warning('下载运单联照片超时，请重新操作')
      }
      ...
      API.getWaybillMaterialUrl().then(res => {
        ...
        if (data) {
          ...
          if (data.length > 0) {
            ...
            this.$message.warning('无运单联资料，不可下载')
          }
          this.downLoading = false
        } else {
    ...
    async onDownLoadFiles(res) {
      if (res.code === 0) {
        ...
        if (filesUrl.length > 0) {
          ...
          this.$message.warning('该运单不存在运单联图片')
        }
      }
      this.downLoading = false
    }

<template>
  <div class='material-panel-receipt'>
    <section class='material-panel-receipt-header'>
      <div class='material-panel-receipt-header-left'>
        <el-radio-group v-model='mode'>
          <el-radio-button label='MaterialPanelImages'>回单图片</el-radio-button>
          <el-radio-button :disabled='!route' label='MaterialPanelRouter'>回单路由</el-radio-button>
        </el-radio-group>
      </div>
      <div class='material-panel-receipt-header-right'>
        <el-button size='small' type='text' @click='onFeedbackClick'>
          <ky-icon style='margin-right: 4px' type='iconmaterial-download' />
          <span>反馈</span>
        </el-button>
        <div class='material-panel-download'>
          <el-button size='small' :loading='downLoading' type='text' @click='createWaybillMaterial'>
            <ky-icon style='margin-right: 4px' type='icondownload' />
            <span>下载全部回单</span>
          </el-button>
        </div>
      </div>
    </section>
    <section v-loading='loading' class='material-panel-receipt-body'>
      <keep-alive>
        <component ref='receiptKeepAliveRef' :is='mode' :images='images' :loading='loading' v-bind='$attrs' />
      </keep-alive>
    </section>
    <material-panel-feedback ref='materialPanelFeedback' />
  </div>
</template>
<script>

...
function renderTimePanel (h, _vm) {
  if (_vm.goodsBatchTimes.length > 0) {
    return renderBatchTimesPanel(h, _vm)
  }
...
function renderBatchTimesPanel (h, _vm) {
  const { dateTimeLabel, datetimePanelValue, hourList, minuteList, secondList, goodsBatchList } = _vm
  return [
    h('div', {
      class: 'vxe-input--time-picker-header'
    }, [
    ...
    h('div', {
      ...
      h('ul', {
        class: 'vxe-input--time-picker-hour-list vxe-input-batch-list'
      }, goodsBatchList.map((item, index) => {
        return h('li', {
          key: index,
          class: {
            'is--selected': datetimePanelValue && `${datetimePanelValue.getHours()}:${datetimePanelValue.getMinutes() < 10 ? `0${datetimePanelValue.getMinutes()}` : datetimePanelValue.getMinutes()}` === item.value
          },
          on: {
            click: evnt => _vm.dateBatchTimeEvent(evnt, item)
          }
        }, item.label)
      })),
      // h('ul', {
...
function renderPanel (h, _vm) {
  ...
  if (isDatePickerType) {
    if (type === 'datetime') {
      ...
      renders.push(
        h('div', {
          class: 'vxe-input--panel-bottom-wrapper'
        }, [
          h('button', {
            class: 'vxe-input--time-picker-confirm',
            attrs: {
    ...
    return h('div', {
      ref: 'panel',
      class: ['vxe-table--ignore-clear vxe-input--panel', className, `type--${type}`, {
        [`size--${vSize}`]: vSize,
...
function renderDatePickerIcon (h, _vm) {
  return h('span', {
    ...
    h('i', {
      class: ['vxe-input--date-picker-icon', 'iconfont icon-calendar']
    })
  ])
}
...
export default {
  ...
  props: {
    hourArray:[Array],
    minuteArray:[Array],
    closeWhenDateChange:{type:Boolean,default:true},
    ...
    goodsBatchTimes: { type: Array, default: () => [] }
  },
  data () {
    return {
      panelIndex: 0,
  ...
  computed: {
    ...
    dateTimeLabel () {
      ...
      if (datetimePanelValue) {
        return XEUtils.toDateString(datetimePanelValue, 'HH:mm')
      }
      return ''
    },
    hmsTime () {
    ...
    dateHeaders () {
      if (this.isDatePickerType) {
        return this.weekDatas.map(day => {
          return {
            value: day,
            label: GlobalConfig.i18n(`vxe.input.date.weeks.w${day}`).replace('周','')
          }
        })
      }
      return []
    ...
    goodsBatchList () {
      return this.goodsBatchTimes.map(item => {
        return {
          label: item,
          value: item
        }
  ...
  methods: {
    ...
    dateSelectItem (date) {
      ...
      console.log('type, datePanelType :>> ', type, datePanelType)
    ...
    dateBatchTimeEvent (event, item) {
      const timeString = item.value.split(':')
      this.datetimePanelValue.setHours(timeString[0])
      this.datetimePanelValue.setMinutes(timeString[1])
      this.dateTimeChangeEvent(event)
    },
    dateHourEvent (evnt, item) {
      this.datetimePanelValue.setHours(item.value)
      this.dateTimeChangeEvent(evnt)
      this.$emit('dateHourChange', item.value)
    },
    dateConfirmEvent () {
    ...
    updateTimePos (liElem) {
      if (liElem) {
        ...
        if (liElem.parentNode.lastElementChild.textContent === liElem.textContent) {

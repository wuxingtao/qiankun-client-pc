<template>
  <div class="waybill-table-container">
    ...
    <ky-table :data="tableData" class="element-table" :paginationVisible="true" :pagination="pagination" @pagination-change="queryData" :height="'calc(100vh - 546px)'" ref="kyTable" v-loading="loading">
...
import { exportExcel ,getSizeListText} from '@utils/clientUtil'
import { getExpressWaybillsByPaymentWithPages, getExpressWaybillsByPayment } from '@/services/api/report'
...
const tableColumns = [
  ...
  { prop: 'deliveryPhone', text: '寄件电话', width: '160' },
...
export default {
  ...
  methods: {
    ...
    async queryData () {
      ...
      this.loading=true
      try{
        const res = await getExpressWaybillsByPaymentWithPages(params)
        if (res.code === 0) {
          res.data.rows.forEach(r=>{
            r.goodsSize=getSizeListText(r.goodsSize)
          })
    ...
    async exportWaybillData () {
      ...
      const res = await getExpressWaybillsByPayment(params)
      if (res.code === 0) {
        ...
        try {
          exportExcel({
            theadColumns,
            jsonData: res.data,
            filename: `快件统计详情${dayjs().format('YYYY-MM-DD HHmmss')}`,
          })
        } catch (ex) {

...
class PrintUtil{
  ...
  //转换为A4模板
  convertLocalPrintDataListForA4Stub(localPrintDataList,number=4,templateType='A4Stub'){
  ...
  handleFieldsInLine(line,index){
    ...
    while (( result = regex.exec(line.field)))  {
      ...
      if(line.barcodeField){
        line.barcodeField = line.barcodeField.replace(line.barcodeField,`~{${line.barcodeField.replace('~{','').replace('}','')}_${index}}`)
      }
    }
  }
}

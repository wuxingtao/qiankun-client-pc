<template>
  <div class="address-container">
    <el-form ref='form' inline :model="formData" :rules="formRules">
          <el-form-item :error="errorMsgOfdistrict" style="margin: 0; padding: 0;">
            <tabs-cascader :value="value" :props="tabsCascaderProps" :labelList="formData.districtList" @valueChange="changeDistrict" :fetch-suggestions="getMultiLevelThinkList" @addressLoaded="addressLoaded" ref="cascaderRef"/>
          </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  watch: {
    formData: {
      deep: true,
      handler: function () {
        const fullAddress = this.formData.districtList[this.formData.districtList.length-1] || ''
        // console.log('addressItem:update:fullAddress :>> ', fullAddress)
        this.$emit('input', fullAddress)
        this.$emit('select',{...this.formData})
      }
    }
  },
  methods: {
